...
const View = ({addResponseFromHome,deleteResponseFromCategory,setDeleteResponseFromView}) => {
  ...
  const categoryVideoDropOverView = async (e)=>{
    ...
    const updatedCatgoryVideoList = categoryDetails?.allVideos?.filter(item=>item.id!=video?.id)
    const updatedCategory = {...categoryDetails,allVideos:updatedCatgoryVideoList}
